(function (global) {
  const labels = {
    IDLE: "未开始", QUEUED: "排队中", PENDING: "等待处理", RUNNING: "处理中",
    READY: "就绪", SUCCESS: "成功", PARTIAL: "部分完成", DEGRADED: "降级",
    FAILED: "失败", STALE: "待更新", DISABLED: "已停用", INTERRUPTED: "已中断",
    CHUNKED: "已分块", EMBEDDING: "向量化", INDEXING: "写入索引", EXCLUDED: "已排除"
  };
  const glyphs = {
    IDLE: "○", QUEUED: "◷", PENDING: "◷", RUNNING: "◐", READY: "✓", SUCCESS: "✓",
    PARTIAL: "◒", DEGRADED: "△", FAILED: "!", STALE: "↻", DISABLED: "—",
  ...
  global.NexusStatus = {
    ...
    tone(status) {
      ...
      if (["RUNNING", "EMBEDDING", "INDEXING", "QUEUED", "PENDING"].includes(status)) return "active";
