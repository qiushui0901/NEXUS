...
- 关联需求：模块页尚未绑定需求证据（`ModuleFactBundle.requirements` 未接入需求检索）；
- 真实 CI 测试报告：测试知识仅来自符号识别（`TEST_SYMBOL`），未关联真实测试执行报告；
- RPC / 消息 / 缓存事实：入口识别覆盖 HTTP/消息/定时注解与 DAO/Repository/Cache 命名启发式，RPC 方法与缓存键规则未结构化抽取；
- `INFERRED` Claim 只有支持状态，没有独立置信度字段（文档 §7.2「显示置信度」待落地）；
- 端到端验收的符号图快照在本机通过同步 commit 完成（真实环境由代码索引生成，受 LLM 网关可达性限制）。
