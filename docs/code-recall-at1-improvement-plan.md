...
| 指标 | 旧基线（旧索引） | 基线 A3（新索引·开关全关） | 实验 E10（开关全开·最终） |
|---|---:|---:|---:|
...
| nDCG@10 | — | 0.8083 | **0.9688** |
| P50 / P95 | 351ms / 656ms | 318ms / 444ms | 334ms / 436ms |

> 实测结论（2026-08-14 最终，五轮评审整改后，E10）：实验 E10 相对基线 A3 提升 Recall@1 **+29.6pp**、Recall@5 **+7.4pp**、Recall@10 **+5.6pp**、MRR **+0.196**、nDCG@10 **+0.161**（0.8083 → 0.9688）；**185 条排名变化中 157 条升至 Top1、9 条从 Top1 掉至 2-5 位**（类内语义排序的代价，全部仍在 Top5）；P95 436ms 不劣于基线。
>
